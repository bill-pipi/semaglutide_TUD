...
tabletext <- cbind(c("Outcome",data$blah),
                   ...
                   c("Smoking cessation medications cohort\n# of cases (overall risk)\n",data$control_group),
                   c("        HR (95% CI)",data$CI)
)


pdf(file.path(workdir,"Figure5b.pdf"),  onefile=FALSE, width=14.5, height=5)
...
forestplot(labeltext=tabletext, 
           ...
           title ="12-month negative control outcomes in patients with obesity and TUD (no T2D) \n(comparison between propensity-score matched cohorts: semaglutide vs smoking cessation medications )", 
