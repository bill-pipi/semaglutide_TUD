...
workdir <- "."
#datafile <- file.path(workdir,"Supplement_Figure8a.csv")
#datafile <- file.path(workdir,"Supplement_Figure8b.csv")
datafile <- file.path(workdir,"Supplement_Figure8c.csv")
...
#pdf(file.path(workdir,"Supplement_Figure8a.pdf"),  onefile=FALSE, width=13, height=6)
#pdf(file.path(workdir,"Supplement_Figure8b.pdf"),  onefile=FALSE, width=13, height=6)
pdf(file.path(workdir,"Supplement_Figure8c.pdf"),  onefile=FALSE, width=13, height=6)
...
forestplot(labeltext=tabletext, 
          ...
          #title ="Overall medical encounters in patients with T2DM and TUD\n(Comparision between matched semaglutide vs other anti-diabetes medications cohorts)", 
          #title ="Overall medical encounters in patients with T2DM and TUD (without obesity)\n(Comparision between matched semaglutide vs other anti-diabetes medications cohorts)", 
          title ="Overall medical encounters in patients with T2DM and TUD (with obesity)\n(Comparision between matched semaglutide vs other anti-diabetes medications cohorts)", 
